...
<script>
   ...
   $(function() {
      table = $('.dataTable-cart').DataTable({
         ...
         "serverside": true,
         "ajax": {
            "url": "<?= site_url('operator/sell/data_cart') ?>",
            "type": "GET"
         }
      }).on('draw.dt', function() {
         loadCart($('#disk').val());
      ...
      tableProduct = $('#dataTable-product').DataTable({
         ...
         "ordering": false,
         "ajax": {
            "url": "<?= site_url('operator/product/data') ?>",
            "type": "GET"
         }
      });
   ...
   function changeQty(productID) {
      $.ajax({
         type: "POST",
         url: "<?= site_url('operator/sell/update_cart/') ?>" + productID,
         data: {
   ...
   function deleteItem(productID) {
      if (confirm("apakah yakin hapus data ?")) {
         $.ajax({
            type: "POST",
            url: "<?= site_url('operator/sell/delete_cart/') ?>" + productID,
            success: function(response) {
               table.ajax.reload(function() {
                  loadCart($('#disk').val());
               });
   ...
   function loadCart(diskon = 0) {
      ...
      $.ajax({
         type: "GET",
         url: "<?= site_url('operator/sell/load_cart/') ?>" + diskon + "/" + $('.total').text(),
         dataType: "JSON",
